...
static void waitForOrder ()
{
    ...
    if (semDown (semgid, sh->mutex) == -1) {                                                     
        perror ("error on the down operation for mutex semaphore access (CH)");
        exit (EXIT_FAILURE);
    }
    ...
    if (semUp (semgid, sh->mutex) == -1) {                                                      
        perror ("error on the up operation for mutex semaphore access (CH)");
        exit (EXIT_FAILURE);
    }

    /* STUDENTS CODE */
    if (semUp (semgid, sh->orderReceived) == -1) {                                                      
        perror ("error on the up operation for orderReceived semaphore access (CH)");
        exit (EXIT_FAILURE);
    }
    /* END OF STUDENTS CODE REGION */
}
...
static void processOrder ()
{
    ...
    if (semDown (semgid, sh->waiterRequestPossible) == -1) {                                                     
        perror ("error on the down operation for waiterRequestPossible semaphore access (CH)");
        exit (EXIT_FAILURE);
    }
    /* END OF STUDENTS CODE REGION */
